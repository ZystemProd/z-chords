...
**Note-input mode** (`state.noteInput`, off by default, toggled by the pencil button or `N`, exited with `Escape`) is what gates *writing*: letters, `R` and a blank-staff click only add notes while it is on. Navigation (arrow keys, clicking a note to select it), transposition, deletion, the duration palette and tying all work regardless — the split is "does this write a new note" versus "does this act on what's already there", the same distinction MuseScore's own note-input toggle makes so that browsing a melody with the keyboard can never accidentally add to it.

Two visual cursors appear only in note-input mode, both drawn as plain SVG elements appended to the live score (cleared and redrawn on every `drawStaff`, so they never survive a rebuild stale):

- **The insertion cursor** (`.ms-insert-cursor`, a translucent band) marks where the *next* note actually lands: the slot next to whatever is at the caret (`insertionSlot()`), which is exactly the index `insertEvent` splices into. This is caret-based, not mouse-based — a click's x position has never decided *where in the sequence* a note goes, only its y decided the pitch.
- **The ghost notehead** (`.ms-input-notehead`, plus `.ms-input-ledger` lines when it falls outside the staff) previews the note a click would write. Its Y follows the mouse and *snaps* to the nearest staff line/space (`staffStepFromLocal`'s rounding — the same inversion a real click uses, so the preview can't disagree with the click). Its **X does not follow the mouse at all** — it snaps to `insertionSlot()`, the same x the insertion cursor uses. This was wrong once: the ghost floated with the pointer, which looked like "snap isn't working" because the preview could sit somewhere a click could never actually write to. `tests/suites/melody-note-input.test.mjs` pins both axes so that regression can't come back quietly.

**Step entry by letter.** With the staff focused and note-input mode on, `A`–`G` insert a note, `R` a rest, `1`–`5` pick the note value (positionally — whole through sixteenth, because matching the denominator would need 1,2,4,8 plus something arbitrary for 16), and `.` cycles the dot. The octave is chosen by `midiForLetterNear`: the octave of that letter nearest the previous pitch, ties upward, so typing C D E after a G4 walks around G rather than leaping to octave 4. Letters enter naturals and the arrow keys alter them, the same division the staff click already has. The emptiness guard in the keydown handler sits *below* this block deliberately — typing must work on a melody with no events, which is exactly when you need it.
